...
class ExamModule:
    ...
    def __init__(self, mod_type, content):
        ...
        self.content = [line.rstrip()
                        for line in content if not re.match(r"\s*$", line)]
        # filter content for bangs
        self.bangs = [
            line for line in self.content if re.match(bang_pattern, line)]
        if mod_type == "mc":
            self.bangs += [line for line in self.content if re.match(
                mc_bang_pattern, line)]
    ...
    def mc_tex(self, qcount):
        ...
        for line in content:
            if line not in bangs:
                ...
                # check if indented
                    ...
                    if len(choices) == 0:
                        compile_error(
                            "No answer choices found: " + question_str)
                    mcq = MCQuestion(question_str, point_str,
        ...
        if "intro-height" in options:
            try:
                ...
                compile_error(
                    "Option intro-height in MC module must have numeric value.")
        else:
            page_height_pt = 500
        for question in questions:
            if type(question) == MCQuestion:
                qwidth, choicewidth = (
                    50, 43) if "twocolumn" in options else (100, 80)
        ...
        for line in content:
            if line not in bangs:
                ...
                if not re.match(r"\s*//", line):
                    ...
                    if match:
                        height = int(
                            float(line[match.start()+1:match.end()-1])*16)-12
    ...
    def gen_ans(self, initial_qcount, solutions):
        ...
        if self.mod_type in ["match", "tf", "mc"]:
            ...
            self.ans_key_str = self.gen_frq_ans(
                initial_qcount, solutions, sheet=False)
            if "sheet" in self.ans_options:
                self.ans_sheet_str = self.gen_frq_ans(
                    initial_qcount, solutions, sheet=True)
    ...
    def gen_frq_ans(self, initial_qcount, solutions, sheet):
        ...
        for (hierarchy, height_str, sol) in solutions:
            ...
            topmost_change_ind = 0 if diff[0] != 0 else (
                1 if diff[1] != 0 else 2)
        ...
        while level >= 0:
            ans_str += "{}\\end{{{}s}}\n".format(
                tab_stubs[level], ans_stubs[level][1:])
            level -= 1
        if "twocolumn" in self.ans_options:
            ans_str += "\t\\end{multicols*}\n" \
...
class Exam:
    ...
    def verify_sections(self):
        ...
        for section in sections:
            if section.sec_type == "section":
                ...
                header_count += 1
        assert len(sections) == s_count + cover_count + \
            header_count, "Misidentified section."
        if cover_count > 1:
    ...
    def get_header_info(self):
        ...
        if not header:
            return None
        content = [item.strip()
                   for item in header.content if item.strip() != ""]
        # remove pkg bangs
        content = [item for item in content if not re.match(
            bang_pattern, item)]
        if len(content) != 3:
    ...
    def exam_preamble(self):
        ...
        if header_elems:
            ...
            e1 = header_elems[1] + \
                " - Page \\thepage" if header_elems[1] != "" else ""
            e2 = header_elems[2] + \
                ":\\kern .5 in" if header_elems[2] != "" else ""
            tex_header = "\\header{{{}}}{{{}}}{{{}}}\n".format(e0, e1, e2)
            tex_str += "\n\\pagestyle{head}\n" + tex_header + "\\headrule\n"
        return tex_str
